...
int main( int argc, char* args[] )
{
    ...
    Burbuja b = new Burbuja(load_image("Burbuja.png")0, 0);
    ...
    while( quit == false )
    {
        ...
        while( SDL_PollEvent( &event ) )
        {
            ...
            if(event.type == SDL_MOUSEBUTTONDOWN){
                ...
                int click_y = event.button.y;
                    if (click_x >= b.x && click_x <= b.x + b.surface-> w && click_y >= b.y
                    && click_y <= b.y + b.surface->h){
                        exit(0);
                    }
                }
            }
        ...
        if( keystates[ SDLK_RIGHT ] )
        {
         ...
         apply_surface(burbuja_x, burbuja_y, burbuja, screen, NULL);
         burbuja_x++;
         burbuja_y++;
        //Update the screen
